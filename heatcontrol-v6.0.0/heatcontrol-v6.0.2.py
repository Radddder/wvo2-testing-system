...
from max31855_v2 import MAX31855
from ssd1306 import SSD1306_SPI
from font_v2 import Font
from mlx90640_v4 import I2C, MLX90640, RefreshRate
from sdcard import SDCard
import _thread, os, time, gc
...
def peripheral_init():
    ...
    ##### SPI of SDcard ######
    ##########################
    cs_SDcard = Pin(33, mode=Pin.OUT, value=1)
    spi_SDcard = SoftSPI(baudrate=800000, polarity=1, sck=Pin(13), miso=Pin(12), mosi=Pin(14))
    sd = SDCard(spi_SDcard,cs_SDcard)
    #U3
    ##########################
    #### SPI of MAX31855 #####
    ##########################
    ...
    return led, sd, spi_max31855_samp, cs_max31855_samp, spi_oled, cs_oled, dc_oled, res_oled
...
def relay_init():
    ...
    return relay_1, relay_2, relay_3
...
##### Main Program #######
led, sd, spi_samp, cs_samp, spi_oled, cs_oled, dc_oled, res_oled = peripheral_init()
relay_1, relay_2, relay_3 = relay_init()
...
while(1):
    ...
    oled_print()
    if led():
        _thread.start_new_thread(take_photo,())
        # IRphoto
        mlx.getFrame(frame)
        print(frame)
        print("\n")
