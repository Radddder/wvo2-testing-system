...
def oled_print():
    ...
    oled.text("Rreturn",72,2,1) #Mode
    oled.text("Target:%d to %d"%(Tmin[Loopnow-1],Tmax[Loopnow-1]),0,15,1)
    oled.text("  T_TEC:%.2fC"%temperature_now_tec,0,41,1) #TEC温度
    oled.text(" T_samp:%.2fC"%temperature_now_samp,0,54,1) #Samp温度
    #Status print
    if Systemstate == 0:
...
while(1):   
    if Loopnow <= len(Tmax):
        for Ttarget in range(Tmax[Loopnow],Tmin[Loopnow]-1,-1):
            while(holding_time<=3):
                time.sleep_ms(500)
                led(not led())
                ...
                #command
                holding_control(Ttarget)        
                pc_print()
                oled_print()
                if abs(temperature_now_samp-Ttarget):
                    holding_time = holding_time + 0.5
                if holding_time ==3:
                    relay_3.on() # take a photo
                    relay_3.off() 
                    relay_4.on()
                    relay_4.off()
                    holding_time = 0
